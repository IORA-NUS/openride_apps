...
class GenerateBehavior():
    ...
    stop_locations = {
        coverage_area['name']: BusStop().get_locations_within(coverage_area['districts']) # NOTE need to pass a list
                                            for coverage_area in assignment_settings.get('profile', {}).get('coverage_area', [])
    }
    rng = default_rng()
    ...
    def ridehail_driver(cls, id, record=None):
        ...
        if record is None:
            ...
            coverage_area = choice(assignment_settings.get('profile', {}).get('coverage_area', []))
        ...
        behavior = {
            ...
            'steps_per_action': driver_settings.get('steps_per_action'),
            'response_rate': driver_settings.get('response_rate'),
            'step_only_on_events': driver_settings.get('step_only_on_events'),
            ...
            'profile': {
                ...
                'update_passenger_location': driver_settings.get('profile', {}).get('update_passenger_location'),
                'action_when_free': driver_settings.get('profile', {}).get('action_when_free'),
            },
    ...
    def ridehail_analytics(cls, id, record=None):
        ...
        behavior = {
            ...
            'profile': {
                ...
                'write_ph_output_to_file': analytics_settings.get('profile', {}).get('write_ph_output_to_file', False),
                'paths_history_time_window': analytics_settings.get('profile', {}).get('paths_history_time_window', 0),
            }
        }
