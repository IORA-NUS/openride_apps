...
from apps.ridehail.statemachine import RidehailPassengerTripStateMachine, driver_passenger_interactions
# from apps.ridehail import RideHailActions, validate_assigned_payload
from apps.ridehail.statemachine import RideHailActions, RideHailEvents
from apps.ridehail.message_data_models import AssignedActionPayload, DriverWorkflowPayload
...
class PassengerApp(ORSimApp, DriverInteractionMixin):
    ...
    def __init__(self, run_id, sim_clock, behavior, messenger, agent_helper=None):
        ...
        self.current_loc = self.behavior.get('profile', {}).get('pickup_loc')
    ...
    def launch(self, sim_clock):
        ...
        self.trip.create_new_trip_request(sim_clock, self.current_loc, self.manager.as_dict(), self.behavior.get('profile', {}).get('pickup_loc'), self.behavior.get('profile', {}).get('dropoff_loc'), self.behavior.get('profile', {}).get('trip_price'))
    ...
    def perform_workflow_actions(self):
        ...
        if (trip['state'] == RidehailPassengerTripStateMachine.passenger_requested_trip.name
            and (self.behavior.get('profile', {}).get('trip_request_time', 0) + (self.behavior.get('profile', {}).get('patience', 0) / self.agent_helper.step_size) < self.agent_helper.current_time_step)
        ):
            logging.info(
                ...
                f"Passenger {self.manager.get_id()} has run out of patience. Requested: {self.behavior.get('profile', {}).get('trip_request_time', 0)}, Max patience: {self.behavior.get('profile', {}).get('patience', 0)/self.agent_helper.step_size} steps"
            )
