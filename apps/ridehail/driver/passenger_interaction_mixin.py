...
class PassengerInteractionMixin():
    ...
    def _on_passenger_rejected_trip(self, payload, data):
        ...
        if self.behavior.get('profile', {}).get('action_when_free') == 'random_walk':
            self.active_route, self.projected_path, self.traversed_path = create_route(self.current_loc, self.behavior.get('profile', {}).get('empty_dest_loc')) # self.behavior['empty_dest_loc'])
    ...
    def _on_state_pickedup(self, time_since_last_event):
        if time_since_last_event >= self.behavior.get('profile', {}).get('transition_time_pickup', 0): #self.behavior['transition_time_pickup']:
    ...
    def _on_state_droppedoff(self, time_since_last_event):
        if time_since_last_event >= self.behavior.get('profile', {}).get('transition_time_dropoff', 0): #self.behavior['transition_time_dropoff']:
            # self.trip.end_trip(self.current_time_str, current_loc=self.current_loc)
            self.trip.apply_trip_transition_and_notify(
                transition=RidehailDriverTripStateMachine.end_trip.name,
                data={
