...
class CompromiseServiceBiasMatching(AbstractSolver):
    ''' '''
                ...
                for d in driverListPotential:
                    ...
                    for p in paxList:
                        ...
                        if reducedPickupTime[p['_id'],d['driver']] < self._params['max_travel_time_pickup']:
                            ...
                            #                             (online_params['weight_service_score'] * (d['meta']['profile']['service_score'] * p['trip_price']) / (offline_params['scale_factor_service_score'] *  offline_params['scale_factor_revenue']))
                            c[p['_id'],d['driver']] = (online_params['weight_pickup_time'] * (self._params['offline_params']['ub_pickup_time'] - reducedPickupTime[p['_id'],d['driver']]) / (2 * offline_params['scale_factor_reverse_pickup_time'])) + \
                                                        (online_params['weight_service_score'] * sqrt((d['meta']['profile']['service_score'] * p['trip_price']) / (offline_params['scale_factor_service_score'] *  offline_params['scale_factor_revenue'])))
                        else:
                            c[p['_id'],d['driver']] = 0
    ...
    def update_online_params(self, scale_factor, driver_list, passenger_list, matched_pairs, offline_params, online_params):
        ...
        online_params['exp_target_service_score'] = (scale_factor + 1) * sqrt(offline_params['target_service_score'] * offline_params['target_revenue'])
        ...
        if matched_pairs is not None:
            if len(matched_pairs) !=0:
                ...
                for pair in matched_pairs:
                    ...
                    service_score_step += sqrt(((driver['meta']['profile']['service_score'] * passenger_trip['trip_price']) / (offline_params['scale_factor_service_score'] * offline_params['scale_factor_revenue'])))
