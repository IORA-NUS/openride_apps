...
class GreedyMaxServiceScoreMatching(AbstractSolver):
    ''' '''
    ...
    def solve(self, driver_list, passenger_trip_list, distance_matrix, offline_params, online_params):
        ...
        # # Sort Driver in descending order of service_score
        sorted_idx = [x[0] for x in sorted(enumerate(driver_list), key = lambda d: d[1]['meta']['profile']['service_score'], reverse=True)]
...
if __name__ == '__main__':
    ...
    assignment, matched_pairs = GreedyMaxServiceScoreMatching(None).solve(d_list, p_list, dist_matrix, None)
